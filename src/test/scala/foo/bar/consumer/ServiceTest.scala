...
class ServiceTest extends AnyFreeSpec {
  "Service Example" - {
    "Registering and using services" in {
      ...
      val registryBuilder: RegistryFactories =
        RegistryFactories()
...
object Services {
  ...
  case class ServiceBMaker(override val getInput: String)
    ...
    lazy override protected val getObject: ServiceConstructor[String, ServiceBImpl] = ServiceBImpl
  ...
  case object ServiceCMaker extends ServiceFactory[Unit, ServiceCImpl] {
    ...
    lazy override protected val getObject: ServiceConstructor[Unit, ServiceCImpl] = ServiceCImpl
